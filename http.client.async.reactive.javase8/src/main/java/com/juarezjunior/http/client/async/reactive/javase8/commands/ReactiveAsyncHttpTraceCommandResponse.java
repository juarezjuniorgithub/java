...
public class ReactiveAsyncHttpTraceCommandResponse<T> extends HystrixCommand<CompletionStage<Response>> {
	...
	public ReactiveAsyncHttpTraceCommandResponse(final String targetUrl, final int timeout,
			Object... additionalProperties) {

		super(Setter
				.withGroupKey(HystrixCommandGroupKey.Factory
						.asKey(ReactiveAsyncCommandsEnum.TRACE_COMMAND_RESP_GROUP.value()))
	...
	public ReactiveAsyncHttpTraceCommandResponse(final Link link, final int timeout, Object... additionalProperties) {
	...
	public ReactiveAsyncHttpTraceCommandResponse(final URI uri, final int timeout, Object... additionalProperties) {
	...
	public ReactiveAsyncHttpTraceCommandResponse(final UriBuilder uriBuilder, final int timeout,
			Object... additionalProperties) {
	...
	public ReactiveAsyncHttpTraceCommandResponse(final RxWebTarget<RxCompletionStageInvoker> target, final int timeout,
			Object... additionalProperties) {
	...
	protected CompletionStage<Response> run() throws Exception {
		...
		case STRING_URL:
			...
			response = callbackDispatcher.traceRequestForResponse(stringTarget, informedAdditionalProperties);
			break;

		case LINK:
			ReactiveAsyncHelper.checkLink(linkTarget);
			response = callbackDispatcher.traceRequestForResponse(linkTarget, informedAdditionalProperties);
			break;

		case URI:
			ReactiveAsyncHelper.checkUri(uriTarget);
			response = callbackDispatcher.traceRequestForResponse(uriTarget, informedAdditionalProperties);
			break;

		case URIBUILDER:
			ReactiveAsyncHelper.checkUriBuilder(uriBuilderTarget);
			response = callbackDispatcher.traceRequestForResponse(uriBuilderTarget, informedAdditionalProperties);
			break;

		case WEBTARGET:
			ReactiveAsyncHelper.checkWebTarget(webTarget);
			response = callbackDispatcher.traceRequestForResponse(webTarget, informedAdditionalProperties);
			break;
