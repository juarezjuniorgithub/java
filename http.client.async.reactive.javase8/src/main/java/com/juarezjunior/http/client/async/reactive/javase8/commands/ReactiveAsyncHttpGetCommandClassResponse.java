...
public class ReactiveAsyncHttpGetCommandClassResponse<T> extends HystrixCommand<CompletionStage<T>> {
	...
	public ReactiveAsyncHttpGetCommandClassResponse(final String targetUrlString, final Class<T> responseType,
			final int timeout, Object... additionalProperties) {

		super(Setter
				.withGroupKey(
						HystrixCommandGroupKey.Factory.asKey(ReactiveAsyncCommandsEnum.GET_COMMAND_CLASS_GROUP.value()))
	...
	public ReactiveAsyncHttpGetCommandClassResponse(final Link link, final Class<T> responseType, final int timeout,
	...
	public ReactiveAsyncHttpGetCommandClassResponse(final URI uri, final Class<T> responseType, final int timeout,
			Object... additionalProperties) {
	...
	public ReactiveAsyncHttpGetCommandClassResponse(final UriBuilder uriBuilder, final Class<T> responseType,
			final int timeout, Object... additionalProperties) {
	...
	public ReactiveAsyncHttpGetCommandClassResponse(final RxWebTarget<RxCompletionStageInvoker> target,
	...
	protected CompletionStage<T> run() throws Exception {
		...
		case STRING_URL:
			...
			response = (CompletionStage<T>) callbackDispatcher.getRequestForTypedResponse(stringTarget, clazzType,
					informedAdditionalProperties);
			break;

		case LINK:
			ReactiveAsyncHelper.validateUrlTypeLink(linkTarget, clazzType);
			response = (CompletionStage<T>) callbackDispatcher.getRequestForTypedResponse(linkTarget, clazzType,
					informedAdditionalProperties);
			break;

		case URI:
			ReactiveAsyncHelper.validateUrlTypeUri(uriTarget, clazzType);
			response = (CompletionStage<T>) callbackDispatcher.getRequestForTypedResponse(uriTarget, clazzType,
					informedAdditionalProperties);
			break;

		case URIBUILDER:
			ReactiveAsyncHelper.validateUrlTypeBuilder(uriBuilderTarget, clazzType);
			response = (CompletionStage<T>) callbackDispatcher.getRequestForTypedResponse(uriBuilderTarget, clazzType,
					informedAdditionalProperties);
			break;

		case WEBTARGET:
			ReactiveAsyncHelper.validateUrlTypeWebTarget(webTarget, clazzType);
			response = (CompletionStage<T>) callbackDispatcher.getRequestForTypedResponse(webTarget, clazzType,
					informedAdditionalProperties);
			break;
