...
public class ReactiveAsyncHttpGetCommandGenericTypeResponse<T> extends HystrixCommand<CompletionStage<T>> {
	...
	public ReactiveAsyncHttpGetCommandGenericTypeResponse(final String targetUrl, final GenericType<T> type,
			final int timeout, Object... additionalProperties) {

		super(Setter
				.withGroupKey(
						HystrixCommandGroupKey.Factory.asKey(ReactiveAsyncCommandsEnum.GET_COMMAND_GEN_GROUP.value()))
	...
	public ReactiveAsyncHttpGetCommandGenericTypeResponse(final Link link, final GenericType<T> type, final int timeout,
	...
	public ReactiveAsyncHttpGetCommandGenericTypeResponse(final URI uri, final GenericType<T> type, final int timeout,
			Object... additionalProperties) {
	...
	public ReactiveAsyncHttpGetCommandGenericTypeResponse(final UriBuilder uriBuilder, final GenericType<T> type,
			final int timeout, Object... additionalProperties) {
	...
	public ReactiveAsyncHttpGetCommandGenericTypeResponse(final RxWebTarget<RxCompletionStageInvoker> target,
	...
	protected CompletionStage<T> run() throws Exception {
		...
		case STRING_URL:
			...
			response = (CompletionStage<T>) callbackDispatcher.getRequestForGenericTypedResponse(stringTarget,
					genericType, informedAdditionalProperties);
			break;

		case LINK:
			ReactiveAsyncHelper.validateUrlTypeLink(linkTarget, genericType);
			response = (CompletionStage<T>) callbackDispatcher.getRequestForGenericTypedResponse(linkTarget,
					genericType, informedAdditionalProperties);
			break;

		case URI:
			ReactiveAsyncHelper.validateUrlTypeUri(uriTarget, genericType);
			response = (CompletionStage<T>) callbackDispatcher.getRequestForGenericTypedResponse(uriTarget, genericType,
					informedAdditionalProperties);
			break;

		case URIBUILDER:
			ReactiveAsyncHelper.validateUrlTypeBuilder(uriBuilderTarget, genericType);
			response = (CompletionStage<T>) callbackDispatcher.getRequestForGenericTypedResponse(uriBuilderTarget,
					genericType, informedAdditionalProperties);
			break;

		case WEBTARGET:
			ReactiveAsyncHelper.validateUrlTypeWebTarget(webTarget, genericType);
			response = (CompletionStage<T>) callbackDispatcher.getRequestForGenericTypedResponse(webTarget, genericType,
					informedAdditionalProperties);
			break;
