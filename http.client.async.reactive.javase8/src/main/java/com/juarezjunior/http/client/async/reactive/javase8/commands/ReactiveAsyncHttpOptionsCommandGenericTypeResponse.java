...
public class ReactiveAsyncHttpOptionsCommandGenericTypeResponse<T> extends HystrixCommand<CompletionStage<T>> {
	...
	public ReactiveAsyncHttpOptionsCommandGenericTypeResponse(final String targetUrl, final GenericType<T> type,
			final int timeout, Object... additionalProperties) {

		super(Setter
				.withGroupKey(HystrixCommandGroupKey.Factory
						.asKey(ReactiveAsyncCommandsEnum.OPTIONS_COMMAND_GEN_GROUP.value()))
	...
	public ReactiveAsyncHttpOptionsCommandGenericTypeResponse(final Link link, final GenericType<T> type,
			final int timeout, Object... additionalProperties) {
	...
	public ReactiveAsyncHttpOptionsCommandGenericTypeResponse(final URI uri, final GenericType<T> type,
			final int timeout, Object... additionalProperties) {
	...
	public ReactiveAsyncHttpOptionsCommandGenericTypeResponse(final UriBuilder uriBuilder, final GenericType<T> type,
			final int timeout, Object... additionalProperties) {
	...
	public ReactiveAsyncHttpOptionsCommandGenericTypeResponse(final RxWebTarget<RxCompletionStageInvoker> target,
	...
	protected CompletionStage<T> run() throws Exception {
		...
		case STRING_URL:
			...
			response = (CompletionStage<T>) callbackDispatcher.optionsRequestForGenericTypedResponse(stringTarget,
					genericType, informedAdditionalProperties);
			break;

		case LINK:
			ReactiveAsyncHelper.validateUrlTypeLink(linkTarget, genericType);
			response = (CompletionStage<T>) callbackDispatcher.optionsRequestForGenericTypedResponse(linkTarget,
					genericType, informedAdditionalProperties);
			break;

		case URI:
			ReactiveAsyncHelper.validateUrlTypeUri(uriTarget, genericType);
			response = (CompletionStage<T>) callbackDispatcher.optionsRequestForGenericTypedResponse(uriTarget,
					genericType, informedAdditionalProperties);
			break;

		case URIBUILDER:
			ReactiveAsyncHelper.validateUrlTypeBuilder(uriBuilderTarget, genericType);
			response = (CompletionStage<T>) callbackDispatcher.optionsRequestForGenericTypedResponse(uriBuilderTarget,
					genericType, informedAdditionalProperties);
			break;

		case WEBTARGET:
			ReactiveAsyncHelper.validateUrlTypeWebTarget(webTarget, genericType);
			response = (CompletionStage<T>) callbackDispatcher.optionsRequestForGenericTypedResponse(webTarget,
					genericType, informedAdditionalProperties);
			break;
