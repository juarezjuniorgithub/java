...
public class ReactiveAsyncHttpTraceCommandClassResponse<T> extends HystrixCommand<CompletionStage<T>> {
	...
	public ReactiveAsyncHttpTraceCommandClassResponse(final String targetUrlString, final Class<T> responseType,
			final int timeout, Object... additionalProperties) {

		super(Setter
				.withGroupKey(HystrixCommandGroupKey.Factory
						.asKey(ReactiveAsyncCommandsEnum.TRACE_COMMAND_CLASS_GROUP.value()))
	...
	public ReactiveAsyncHttpTraceCommandClassResponse(final Link link, final Class<T> responseType, final int timeout,
	...
	public ReactiveAsyncHttpTraceCommandClassResponse(final URI uri, final Class<T> responseType, final int timeout,
			Object... additionalProperties) {
	...
	public ReactiveAsyncHttpTraceCommandClassResponse(final UriBuilder uriBuilder, final Class<T> responseType,
			final int timeout, Object... additionalProperties) {
	...
	public ReactiveAsyncHttpTraceCommandClassResponse(final RxWebTarget<RxCompletionStageInvoker> target,
	...
	protected CompletionStage<T> run() throws Exception {
		...
		case STRING_URL:
			...
			response = (CompletionStage<T>) callbackDispatcher.traceRequestForTypedResponse(stringTarget, clazzType,
					informedAdditionalProperties);
			break;

		case LINK:
			ReactiveAsyncHelper.validateUrlTypeLink(linkTarget, clazzType);
			response = (CompletionStage<T>) callbackDispatcher.traceRequestForTypedResponse(linkTarget, clazzType,
					informedAdditionalProperties);
			break;

		case URI:
			ReactiveAsyncHelper.validateUrlTypeUri(uriTarget, clazzType);
			response = (CompletionStage<T>) callbackDispatcher.traceRequestForTypedResponse(uriTarget, clazzType,
					informedAdditionalProperties);
			break;

		case URIBUILDER:
			ReactiveAsyncHelper.validateUrlTypeBuilder(uriBuilderTarget, clazzType);
			response = (CompletionStage<T>) callbackDispatcher.traceRequestForTypedResponse(uriBuilderTarget, clazzType,
					informedAdditionalProperties);
			break;

		case WEBTARGET:
			ReactiveAsyncHelper.validateUrlTypeWebTarget(webTarget, clazzType);
			response = (CompletionStage<T>) callbackDispatcher.traceRequestForTypedResponse(webTarget, clazzType,
					informedAdditionalProperties);
			break;
